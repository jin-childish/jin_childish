...
var DBbutton = {};
	DBbutton["setbackgroundColor"] = function(color)
	{$('.DB').css("background-color",color);};//옆의 코드는 jquery를 이용하여 단축한 것이다. jquery를 연동한 html에서만 작동함.
var colorchip = {
	"Pink" : "#D16893",
};
...
//		while(k < alist.length){alist[k].style.color = color; k=k+1;}
	...
	$('a').css("color", color);// 옆의 코드는 jquery를 이용하여 단축한 것이다. jquery를 연동한 html에서만 작동함.
}
function nightDayShift(SELF){								   
							...
							Body.setcolor('white');
							document.querySelector('.Book_description').style.color = 'black';
								//Book_description이 body태그에 종속되어 있기 때문에 폰트색상변경이 일어나서 그것을 교정한다.
							Header.setbordercolor('white');
							DBbutton.setbackgroundColor('white');
							SELF.value = 'DAY.ver';
							...
							DBbutton.setbackgroundColor(colorchip.Pink);
  ...
  // 원래는 document라는 object에서 querySelector 또는 querySelectorAll 같은 method를 이용하여 함수를 설계해야 하나 refactoring을 위해서 각 중요 태그 중 앞자리만 대문자로 한 변수를 object로 정의한 후 각 method를 만들어서 정의한 후 그것을 사용하였다. //// JavaScript Document